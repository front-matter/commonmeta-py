...
def test_journal_article():
    ...
    assert schema_org.get('author')[0] == {
        '@type': 'Perso', 'name': 'Martial Sankar'}
    ...
    assert schema_org.get('publisher') == {
        '@type': 'Organization', 'name': 'eLife Sciences Publications, Ltd'}
    ...
    assert schema_org.get('isPartOf') == {
        '@type': 'Periodical', 'issn': '2050-084X'}
    assert schema_org.get('periodical') == {'identifier': '2050-084X', 'identifierType': 'ISSN', 'volume': '3', '@type': 'Journal', 'name': 'eLife'}

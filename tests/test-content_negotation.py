...
def test_content_negotiation_citeproc_json():
    ...
    assert str == str(res).__class__
    citeproc =  json.loads(res)
    assert citeproc["container-title-short"] == "Science"
...
def test_content_negotiation_alt_url():
    ...
    assert str == str(res).__class__
    assert res.startswith("@article{Frank_1970")
...
def test_content_negotiation_style():
    ...
    assert res_apa == "Frank, H. S. (1970). The Structure of Ordinary Water. Science, 169(3946), 635–641. https://doi.org/10.1126/science.169.3946.635\n"
    res_ieee = cn.content_negotiation(
        ids="10.1126/science.169.3946.635", format="text", style="ieee"
    )
    assert res_ieee == "[1]H. S. Frank, “The Structure of Ordinary Water,” Science, vol. 169, no. 3946, pp. 635–641, Aug. 1970, doi: 10.1126/science.169.3946.635.\n"

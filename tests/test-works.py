...
import vcr
import requests
from talbot import exceptions, Crossref
...
def test_works_with_one_id():
    ...
    assert "10.1371/journal.pone.0033693" == res["message"]["DOI"]
    assert "Methylphenidate Exposure Induces Dopamine Neuron Loss and Activation of Microglia in the Basal Ganglia of Mice" == res["message"]["title"][0]
    assert  "Sadasivan" == res["message"]["author"][0]["family"]
    assert  "Shankar" == res["message"]["author"][0]["given"]

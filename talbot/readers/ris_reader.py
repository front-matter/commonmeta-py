...
from ..constants import (RIS_TO_CP_TRANSLATIONS, RIS_TO_DC_TRANSLATIONS, RIS_TO_SO_TRANSLATIONS)
...
#                                                                                    :sandbox, :validate, :ra))
    ...
    if meta.get('Y1', None) is not None:
        dates.append({'date': get_date_from_parts(*created_date_parts), 'dateType': 'Created'})
    ...
    state = meta.get('DO', None).present? || read_options.present? ? 'findable' : 'not_found'

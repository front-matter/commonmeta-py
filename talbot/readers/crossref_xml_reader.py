"""crossref_xml reader for Talbot"""
...
def read_crossref_xml(data: Optional[dict], **kwargs) -> TalbotMeta:
    """read_crossref_xml"""
    if data is None:
        return {"state": "not_found"}
    ...
    if py_.get(meta, "crossref.journal", None):
        ...
        program_metadata = py_.get(
            bibliographic_metadata,
            "crossmark.custom_metadata.program",
            bibliographic_metadata.get("program", None),
        )
        if journal_article:
    ...
    license_ = next(
        (
            i
            for i in wrap(py_.get(bibliographic_metadata, "program.license_ref"))
            if i.get("@applies_to", None) == "vor"
        ),
        {},
    ).get("#text", None)
    ...
    if journal_metadata:
        issn = normalize_issn(py_.get(journal_metadata, "issn.#text"))
        container = compact(
            {
                "type": "Journal",
                "identifier": issn,
